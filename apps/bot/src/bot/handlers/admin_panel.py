...
from core_shared.services.slot import generate_slots_for_date
from core_shared.services.user import get_user, get_user_count
...
async def admin_panel_handler(
    message: types.Message, user: User, state: FSMContext, session: AsyncSession
):
...
async def show_profile_handler(
    ...
    user = await get_user(tg_id=contact.user_id, session=session)
    if not user:
