...
from core_shared.services.user import create_user, get_user, modify_user

from bot.core.exceptions import InternalError
from bot.general import ask_for_registration
...
async def start(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    if user := await get_user(tg_id=message.from_user.id, session=session):
...
async def register_user(
    ...
    if (user := await get_user(tg_id=tg_id, session=session)) is not None:
        await modify_user(
            user_id=user.id,
            full_name=full_name,
        ...
        user = await create_user(
            tg_id=tg_id, full_name=full_name, phone_number=phone_number, session=session
        )
